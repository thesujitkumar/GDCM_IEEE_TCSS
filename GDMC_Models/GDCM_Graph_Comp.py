...
class DocLSTM(nn.Module):
    ...
    def forward(self, body):
        ...
        Pos_set_graph_node_list=[]
        Sub_Graph_node_list=[]
        for i in range(len(indexs)):
            a = nx.ego_graph(body_graph,indexs[i], radius = self.radius)
            Sub_Graph_node_list.append(torch.tensor(list(a.nodes)))
            Pos_set_graph_node_list.append(list(a.nodes()))

        Pos_set_graph_nodes =  np.concatenate(Pos_set_graph_node_list)
        # print(Pos_set_graph_nodes)

        body_graph_node_list = list(body_graph.nodes)
        # print(body_graph_node_list)

        Postive_graph_complement_nodes_list = torch.tensor(list(set(body_graph_node_list) - set(Pos_set_graph_nodes)))
        # print(body_graph_node_list)

        if len(Postive_graph_complement_nodes_list) == 0:

            Postive_graph_complement_nodes_list = torch.tensor(body_graph_node_list)

        # print(Postive_graph_complement_nodes_list)
        ...
        "Subgraph Embedding for Graph complment = Nodes in Bigram Network - Nodes in psoitiv set"
        Postive_graph_complement_nodes_emb_mat = b[Postive_graph_complement_nodes_list]
        # print(Postive_graph_complement_nodes_emb_mat.shape)

        s_max, ind= torch.max(Postive_graph_complement_nodes_emb_mat,dim=0)
        s_min, ind= torch.min(Postive_graph_complement_nodes_emb_mat,dim=0)
        s_sum = torch.mean(Postive_graph_complement_nodes_emb_mat,dim=0)
        pos_graph_comp_features = torch.cat((s_max,s_min,s_sum),0).view(1,96)
        # print(pos_graph_comp_features.shape)
        ...
        del Postive_graph_complement_nodes_emb_mat,Postive_graph_complement_nodes_list, body_graph_node_list, Pos_set_graph_nodes,Sub_Graph_node_list, subgraph_mat, subgraph_pooled
        return  head_pooled_feature, pos_body_multi_head_rep,pos_graph_comp_features , Global_body_graph_rep
